/* pwrkey-mca-cc8x.c - Power Key device driver for MCA on ConnectCore 8X
...
static irqreturn_t mca_cc8x_pwrkey_power_off_irq_handler(int irq, void *data)
{
	struct mca_cc8x_pwrkey *pwrkey = data;
...
static irqreturn_t mca_cc8x_pwrkey_sleep_irq_handler(int irq, void *data)
{
	struct mca_cc8x_pwrkey *pwrkey = data;
...
static int mca_cc8x_pwrkey_initialize(struct mca_cc8x_pwrkey *pwrkey)
{
	int ret;
	uint8_t pwrctrl0 = 0;

	ret = regmap_write(pwrkey->mca->regmap, MCA_CC8X_PWR_KEY_DEBOUNCE,
	...
	ret = regmap_write(pwrkey->mca->regmap, MCA_CC8X_PWR_KEY_DELAY,
	...
	ret = regmap_write(pwrkey->mca->regmap, MCA_CC8X_PWR_KEY_GUARD,
	...
	if (pwrkey->key_power)
		pwrctrl0 |= MCA_CC8X_PWR_KEY_OFF_EN;

	if (pwrkey->key_sleep)
		pwrctrl0 |= MCA_CC8X_PWR_KEY_SLEEP_EN;

	if (pwrkey->pwroff_guard_sec != 0)
		pwrctrl0 |= MCA_CC8X_PWR_GUARD_EN;

	ret = regmap_write(pwrkey->mca->regmap, MCA_CC8X_PWR_CTRL_0, pwrctrl0);
	if (ret < 0) {
		dev_err(pwrkey->mca->dev,
...
static int of_mca_cc8x_pwrkey_read_settings(struct device_node *np,
					     struct mca_cc8x_pwrkey *pwrkey)
{
	uint32_t val;
...
static int mca_cc8x_pwrkey_probe(struct platform_device *pdev)
{
	...
	pwrkey = devm_kzalloc(&pdev->dev, sizeof(struct mca_cc8x_pwrkey),
			     GFP_KERNEL);
	if (!pwrkey) {
	...
	pwrkey->irq_power = platform_get_irq_byname(pdev,
					MCA_CC8X_IRQ_PWR_OFF_NAME);
	pwrkey->irq_sleep = platform_get_irq_byname(pdev,
					MCA_CC8X_IRQ_PWR_SLEEP_NAME);
	pwrkey->input->name = MCA_CC8X_DRVNAME_PWRKEY;
	pwrkey->input->phys = MCA_CC8X_DRVNAME_PWRKEY "/input0";
	...
	ret = of_mca_cc8x_pwrkey_read_settings(np, pwrkey);
	if (ret) {
		dev_err(&pdev->dev, "Failed to get %s dtb settings\n",
			MCA_CC8X_DRVNAME_PWRKEY);
		goto err_free_inputdev;
	}

	ret = mca_cc8x_pwrkey_initialize(pwrkey);
	if (ret) {
	...
	if (pwrkey->key_power) {
		ret = devm_request_threaded_irq(&pdev->dev, pwrkey->irq_power, NULL,
						mca_cc8x_pwrkey_power_off_irq_handler,
						IRQF_TRIGGER_LOW | IRQF_ONESHOT,
						MCA_CC8X_IRQ_PWR_OFF_NAME, pwrkey);
		if (ret) {
			dev_err(&pdev->dev, "Failed to request %s IRQ (%d).\n",
				MCA_CC8X_IRQ_PWR_OFF_NAME, pwrkey->irq_power);
			goto err_free_inputdev;
		}
	}
	...
	if (pwrkey->key_sleep) {
		ret = devm_request_threaded_irq(&pdev->dev, pwrkey->irq_sleep, NULL,
						mca_cc8x_pwrkey_sleep_irq_handler,
						IRQF_TRIGGER_LOW | IRQF_ONESHOT,
						MCA_CC8X_IRQ_PWR_SLEEP_NAME, pwrkey);
		if (ret) {
			dev_err(&pdev->dev, "Failed to request %s IRQ (%d).\n",
				MCA_CC8X_IRQ_PWR_SLEEP_NAME, pwrkey->irq_sleep);
			goto err_irq1;
		}
...
static int mca_cc8x_pwrkey_remove(struct platform_device *pdev)
{
	struct mca_cc8x_pwrkey *pwrkey = platform_get_drvdata(pdev);
...
#ifdef CONFIG_PM_SLEEP
static int __maybe_unused mca_cc8x_pwrkey_resume(struct device *dev)
{
...
static int __maybe_unused mca_cc8x_pwrkey_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct mca_cc8x_pwrkey *pwrkey = platform_get_drvdata(pdev);
	unsigned long flags;
...
static SIMPLE_DEV_PM_OPS(mca_cc8x_pwrkey_pm_ops, mca_cc8x_pwrkey_suspend, mca_cc8x_pwrkey_resume);
#endif /* CONFIG_PM_SLEEP */

static const struct of_device_id mca_cc8x_pwrkey_ids[] = {
        { .compatible = "digi,mca-cc8x-pwrkey", },
        { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, mca_cc8x__ids);

static struct platform_driver mca_cc8x_pwrkey_driver = {
	.probe	= mca_cc8x_pwrkey_probe,
	.remove	= mca_cc8x_pwrkey_remove,
	.driver	= {
		.name	= MCA_CC8X_DRVNAME_PWRKEY,
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(mca_cc8x_pwrkey_ids),
#ifdef CONFIG_PM_SLEEP
		.pm	= &mca_cc8x_pwrkey_pm_ops,
#endif
	},
};

static int __init mca_cc8x_pwrkey_init(void)
{
	return platform_driver_register(&mca_cc8x_pwrkey_driver);
}
module_init(mca_cc8x_pwrkey_init);

static void __exit mca_cc8x_pwrkey_exit(void)
{
	platform_driver_unregister(&mca_cc8x_pwrkey_driver);
}
module_exit(mca_cc8x_pwrkey_exit);

MODULE_AUTHOR("Digi International Inc");
MODULE_DESCRIPTION("pwrkey device driver for MCA of ConnectCore 8X");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:" MCA_CC8X_DRVNAME_PWRKEY);
