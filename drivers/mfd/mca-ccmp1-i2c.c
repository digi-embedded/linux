...
static const struct regmap_range mca_ccmp1_readable_ranges[] = {
};

static const struct regmap_range mca_ccmp1_writeable_ranges[] = {
	...
	regmap_reg_range(MCA_CCMP1_MPU_NVRAM_START, MCA_CCMP1_MPU_NVRAM_END),
};

static const struct regmap_range mca_ccmp1_volatile_ranges[] = {
...
static const struct regmap_access_table mca_ccmp1_readable_table = {
	.yes_ranges = mca_ccmp1_readable_ranges,
	.n_yes_ranges = ARRAY_SIZE(mca_ccmp1_readable_ranges),
};

static const struct regmap_access_table mca_ccmp1_writeable_table = {
	.yes_ranges = mca_ccmp1_writeable_ranges,
	.n_yes_ranges = ARRAY_SIZE(mca_ccmp1_writeable_ranges),
};

static const struct regmap_access_table mca_ccmp1_volatile_table = {
	.yes_ranges = mca_ccmp1_volatile_ranges,
	.n_yes_ranges = ARRAY_SIZE(mca_ccmp1_volatile_ranges),
};

static struct regmap_config mca_ccmp1_regmap_config = {
	.reg_bits = 16,
	.val_bits = 8,
	.max_register = 0xFFFF,

	.rd_table = &mca_ccmp1_readable_table,
	.wr_table = &mca_ccmp1_writeable_table,
	.volatile_table = &mca_ccmp1_volatile_table,

	.cache_type = REGCACHE_NONE,
};

static const struct of_device_id mca_ccmp1_dt_ids[] = {
	{ .compatible = "digi,mca-ccmp1", },
	{ }
};
MODULE_DEVICE_TABLE(of, mca_ccmp1_dt_ids);

static int mca_ccmp1_i2c_probe(struct i2c_client *i2c,
			       const struct i2c_device_id *id)
{
	...
	mca->regmap = devm_regmap_init_i2c(i2c, &mca_ccmp1_regmap_config);
	...
	return mca_ccmp1_device_init(mca, i2c->irq);
}

static int mca_ccmp1_i2c_remove(struct i2c_client *i2c)
{
	struct mca_drv *mca = i2c_get_clientdata(i2c);

	mca_ccmp1_device_exit(mca);

	return 0;
}

#ifdef CONFIG_PM
static int mca_ccmp1_i2c_suspend(struct device *dev)
{
	return mca_ccmp1_suspend(dev);
}

static int mca_ccmp1_i2c_resume(struct device *dev)
{
	return mca_ccmp1_resume(dev);
}

/*
 * Use suspend_late/resume_early so the mca_ccmp1 continues being functional
 * during the regular suspend/resume callbacks of other drivers, just in case
 * they use any functionality of the mca.
 */
static const struct dev_pm_ops mca_ccmp1_i2c_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(mca_ccmp1_i2c_suspend, mca_ccmp1_i2c_resume)
};
#endif

static const struct i2c_device_id mca_ccmp1_i2c_id[] = {
        {"mca_ccmp1", 0},
        {},
};
MODULE_DEVICE_TABLE(i2c, mca_ccmp1_i2c_id);

static struct i2c_driver mca_ccmp1_i2c_driver = {
	.driver = {
		.name = "mca_ccmp1",
		.of_match_table = of_match_ptr(mca_ccmp1_dt_ids),
#ifdef CONFIG_PM
		.pm = &mca_ccmp1_i2c_pm_ops,
#endif
	},
	.probe    = mca_ccmp1_i2c_probe,
	.remove   = mca_ccmp1_i2c_remove,
	.id_table = mca_ccmp1_i2c_id,
};

module_i2c_driver(mca_ccmp1_i2c_driver);
