...
static const struct regmap_irq mca_ccmp1_irqs[] = {
	/* MCA irqs A register */
...
static const struct regmap_irq_chip mca_ccmp1_irq_chip = {
	.name = "mca-ccmp1-irq",
	.irqs = mca_ccmp1_irqs,
	.num_irqs = ARRAY_SIZE(mca_ccmp1_irqs),
...
int mca_ccmp1_irq_init(struct mca_drv *mca)
{
	...
	ret = regmap_add_irq_chip(mca->regmap, mca->chip_irq,
				  ...
				  mca->irq_base, &mca_ccmp1_irq_chip,
				  &mca->regmap_irq);
	if (ret) {
