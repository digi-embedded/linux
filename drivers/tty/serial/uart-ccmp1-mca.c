/* uart-ccmp1-mca.c - UART driver for MCA devices.
 ...
 * than the one at /sys/bus/i2c/devices/0-007e/uart-ccmp1-mca/power/wakeup.
 */
struct uart_match {
	struct uart_port *port;
...
static int mca_ccmp1_uart_suspend(struct device *d)
{
	int ret;
...
static const struct dev_pm_ops mca_ccmp1_uart_pm_ops = {
	.suspend	= mca_ccmp1_uart_suspend,
	.resume		= NULL,
	.poweroff	= NULL,
};
#endif
...
MODULE_DESCRIPTION("UART for MCA of ConnectCore MP1");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:" MCA_CCMP1_DRVNAME_UART);
