...
static inline void do_standby(void)
{
    ...
    reg_clkctrl_clkseq = __raw_readl(mx28_virt_addr.clkctrl_addr + HW_CLKCTRL_CLKSEQ);

    reg_clkctrl_xtal = __raw_readl(mx28_virt_addr.clkctrl_addr + HW_CLKCTRL_XTAL);
    ...
    __raw_writel(reg_clkctrl_clkseq, mx28_virt_addr.clkctrl_addr + HW_CLKCTRL_CLKSEQ);
    __raw_writel(reg_clkctrl_xtal, mx28_virt_addr.clkctrl_addr + HW_CLKCTRL_XTAL);
...
void __init mxs_pm_init(void)
{
	...
	if (of_machine_is_compatible("fsl,imx28")){
        ...
        get_virt_addr("interrupt-controller",&mx28_virt_addr.icoll_addr);
